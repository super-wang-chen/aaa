 ...
 * */
function get_config(){
	if($a=cache('site_config')){
		return $a;
	}else{
		$result=Db::name('config')->field('k,v')->select();
		cache('site_config',$result);
		return $result;
	}
}
...
function add($table,$data){
	return Db::name($table)->insert($data,false,true);
}

//更新
function update($table,$data){
	return Db::name($table)->update($data);
}

//获取单页信息
function get_info($table,$map){
	return Db::name($table)->where($map)->find();
}


function get_result($Model, $map = array(), $field = '', $order = '', $limit = 0, $group = '', $having = '') {
    $result=Db::name($Model)->where($map) -> field($field) -> order($order) -> group($group) -> having($having) -> limit($limit) -> select();
    return $result;
}
