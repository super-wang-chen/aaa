<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:87:"D:\freehostnew\canadalotter\web\public/../application/admin\view\index\add_article.html";i:1509589299;s:79:"D:\freehostnew\canadalotter\web\public/../application/admin\view\index\pic.html";i:1509589300;}*/ ?>
<script src="/static/js/jedate/jedate.js"></script>
<link href="/static/js/jedate/skin/green.css" rel='stylesheet' type='text/css' />
<div class="row">
	<div class="col-lg-12">
		<h3 class="page-header"><i class="fa fa-user"></i>添加文章</h3>
		<ol class="breadcrumb">
			...
			<li><i class="fa fa-user"></i>添加文章</li>
		</ol>
	</div>
</div>
<div class="row">
	<div class="col-lg-12">
		<div class="panel panel-default">
			            <div class="panel-heading">
			                <h2><i class="fa fa-indent red"></i><strong>文章信息</strong></h2>
			            </div>
						<div class="panel-body">
							<form action="<?=url('add_article')?>" method="post" enctype="multipart/form-data" class="form-horizontal js_form">
				                <?php
									if(is_array($info)){
								?>
				                ...
				                <div class="form-group">
				                    <label class="col-md-3 control-label" for="text-input">文章来源</label>
				                    <div class="col-md-9">
				                        <input type="text" id="text-input" name="from" value="<?php echo $info['from']; ?>" class="form-control" placeholder="From">
				                        <span class="help-block">标题</span>
				                    </div>
				                </div>				                
				                <div class="form-group">
							        <label class="col-md-3 control-label" for="select">文章分类</label>
							        <div class="col-md-9">
							            <select id="select" name="pid" class="form-control" size="1">
							                <?php if(is_array($cate_list) || $cate_list instanceof \think\Collection): $i = 0; $__LIST__ = $cate_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?>
							                	<option <?php if($v['link']!='news'): ?>disabled<?php endif; if($v['id']==$info['id']){echo 'disabled';} if($v['id']==$info['pid']){echo 'selected';} ?> value="<?php echo $v['id']; ?>"><?php echo $v['title']; ?></option>
							                	<?php if(isset($v['_child'])): if(is_array($v['_child']) || $v['_child'] instanceof \think\Collection): $i = 0; $__LIST__ = $v['_child'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$va): $mod = ($i % 2 );++$i;?>
									                	<option <?php if($va['link']!='news'): ?>disabled<?php endif; if($va['id']==$info['id']){echo 'disabled';}if($va['id']==$info['id']){echo 'disabled';} if($va['id']==$info['pid']){echo 'selected';} ?> value="<?php echo $va['id']; ?>"> - - <?php echo $va['title']; ?></option>
									                	<?php if(isset($va['_child'])): if(is_array($va['_child']) || $va['_child'] instanceof \think\Collection): $i = 0; $__LIST__ = $va['_child'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vb): $mod = ($i % 2 );++$i;?>
											                	<option <?php if($vb['link']!='news'): ?>disabled<?php endif; if($vb['id']==$info['id']){echo 'disabled';}if($vb['id']==$info['id']){echo 'disabled';} if($vb['id']==$info['pid']){echo 'selected';} ?> value="<?php echo $vb['id']; ?>"> - - - <?php echo $vb['title']; ?></option>
											                	<?php if(isset($vb['_child'])): if(is_array($vb['_child']) || $vb['_child'] instanceof \think\Collection): $i = 0; $__LIST__ = $vb['_child'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vc): $mod = ($i % 2 );++$i;?>
													                	<option <?php if($vc['link']!='news'): ?>disabled<?php endif; if($vc['id']==$info['id']){echo 'disabled';}if($vc['id']==$info['id']){echo 'disabled';} if($vc['id']==$info['pid']){echo 'selected';} ?> value="<?php echo $vc['id']; ?>"> - - - - <?php echo $vc['title']; ?></option>
							    ...
							    <div class="form-group">
				                    <label class="col-md-3 control-label">文章重要性</label>
				                    <div class="col-md-9">
				                ...
				                <div class="form-group">
				                    <label class="col-md-3 control-label" for="discount">发表时间</label>
				                    <div class="col-md-2"> 
				                        <input type="text" name="add_time" class="form-control" style="width: 182px;" value="<?php if($info['add_time']){echo date('Y-m-d H:i',$info['add_time']);} ?>" id="start_time" readonly/>
				                    </div>
				                </div>
				                <div class="form-group">
				                ...
				                <div class="form-group">
				                    <label class="col-md-3 control-label" for="textarea-input">简单描述</label>
				                    <div class="col-md-9">
