<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:88:"D:\freehostnew\canadalotter\web\public/../application/admin\view\index\article_list.html";i:1509589299;}*/ ?>
<div class="col-lg-12">
...
<!-- 						<td><?php echo $v['from']; ?></td> -->
						...
						<td>
							<span style="cursor: pointer" data-table="article" data-id="<?php echo $v['id']; ?>" data-key="is_top" data-status="<?php echo $v['is_top']; ?>" class="js_status label label-<?php echo !empty($v['is_top']) && $v['is_top']==1?'success':'default'; ?>"><?php echo !empty($v['is_top']) && $v['is_top']==1?'置顶':'普通'; ?></span>
						<td>
							<span style="cursor: pointer" data-table="article" data-id="<?php echo $v['id']; ?>" data-status="<?php echo $v['status']; ?>" class="js_status label label-<?php echo !empty($v['status']) && $v['status']==1?'success':'default'; ?>"><?php echo !empty($v['status']) && $v['status']==1?'正常':'冻结'; ?></span>
						</td>
						<td>
							<a class="btn btn-info" href="<?=url('add_article',array('id'=>$v['id']))?>">
								<i class="fa fa-edit "></i>
							</a>
							<a class="btn btn-danger js_del" data-table="article" data-id="<?php echo $v['id']; ?>" href="#this">
